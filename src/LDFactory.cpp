...
Entity& LDFactory::createWall(const Vec2i& mPos)
{
    ...
    auto& cPhysics(
    result.createComponent<LDCPhysics>(world, true, mPos, Vec2i{3200, 3200}));
    ...
    assets.tilesetWorld(
    (getRndI(0, 100) < 75) ? Vec2u{1, 0} : Vec2u(3 + getRndI(0, 2), 0)));
...
Entity& LDFactory::createBlock(const Vec2i& mPos, int mVal)
{
    ...
    if(mVal > -1) {
        if(colorMap.find(mVal) == colorMap.end())
            colorMap[mVal] =
            Color(getRndI(0, 255), getRndI(0, 255), getRndI(0, 255), 255);
...
Entity& LDFactory::createBlockBig(const Vec2i& mPos, int mVal)
{
    ...
    // body.onPreUpdate += [&]{ body.setPosition(body.getPosition() - Vec2i{-0,
    // 0}); body.setVelocityX(80); };
    // body.onPostUpdate += [&]{ body.setPosition(body.getPosition() - Vec2i{-0,
    // 0}); body.setVelocityX(80); };
    body.setRestitutionX(0.3f);
    body.setRestitutionY(0.3f);
    body.setMass(10.f);
...
Entity& LDFactory::createPlayer(const Vec2i& mPos)
{
    ...
    auto& cPhysics(
    result.createComponent<LDCPhysics>(world, false, mPos, Vec2i{800, 2700}));
    auto& cRender(result.createComponent<LDCRender>(game, cPhysics.getBody()));
...
Entity& LDFactory::createReceiver(const Vec2i& mPos, int mVal)
{
    auto& result(manager.createEntity());
    auto& cPhysics(
    result.createComponent<LDCPhysics>(world, false, mPos, Vec2i{1600, 1600}));
    cPhysics.setAffectedByGravity(false);
    auto& cRender(result.createComponent<LDCRender>(game, cPhysics.getBody()));
    result.createComponent<LDCBlock>(mVal, game, cPhysics);
...
Entity& LDFactory::createTele(const Vec2i& mPos)
{
    auto& result(manager.createEntity());
    auto& cPhysics(
    result.createComponent<LDCPhysics>(world, false, mPos, Vec2i{1600, 100}));
    cPhysics.setAffectedByGravity(false);
    auto& cRender(result.createComponent<LDCRender>(game, cPhysics.getBody()));
...
Entity& LDFactory::createLift(const Vec2i& mPos, const Vec2f& mVel)
{
    ...
    result.addGroups(LDGroup::Block);
    auto& cPhysics(
    result.createComponent<LDCPhysics>(world, false, mPos, Vec2i{3200, 1800}));
    auto& cRender(result.createComponent<LDCRender>(game, cPhysics.getBody()));
