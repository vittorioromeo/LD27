...
public:
    LDCBlock(sses::Entity& mE, int mVal, LDGame& mGame, LDCPhysics& mCPhysics)
          ...
          ssvu::toStr(val)}
    {
        ...
        body.onResolution += [this](const ResolutionInfo& mRI)
        {
            ...
            if((
               std::abs(body.getVelocity().x) > 400 && mRI.resolution.x != 0) ||
               (std::abs(body.getVelocity().y) > 400 && mRI.resolution.y != 0))
                game.getAssets().playSound("bounce.wav");
        };
        body.onPreUpdate += [this]
        {
            // body.setVelocity(ssvs::getCClamped(body.getVelocity(), -800.f,
            // 800.f));
    ...
    {
        text.setPosition(
        toPixels(body.getShape().getVertexNW<int>()) + ssvs::Vec2f{4, 3});

        if(parent != nullptr) {
            ssvs::Vec2f v{
            (parent->getBody().getPosition() + offset) - body.getPosition()};
            if(ssvs::getDistEuclidean(
               parent->getBody().getPosition(), body.getPosition()) > 1700)
                dropped();
            else
                body.setVelocity(v);
        }
...
public:
          ...
          body(cPhysics.getBody())
    {
        ...
        body.onResolution += [this](const ResolutionInfo& mRI)
        {
            // When you get pushed in a floor/ceiling so hard that a fourth of
    ...
    {
        if(currentBlock != nullptr && !getManager().isAlive(currentBlockStat))
